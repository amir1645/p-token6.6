...
async function connectWallet() {
  if (typeof window.ethers === 'undefined') {
    document.getElementById("message").innerText = "Error: ethers.js failed to load. Please check your internet connection.";
    return;
  }
  ...
  try {
    ...
    if (walletProvider === "metamask") {
      if (!window.ethereum) {
        document.getElementById("message").innerText = "Please install a wallet or use the wallet's built-in browser!";
        return;
      }
      ...
      if (!WalletConnectProvider) {
        document.getElementById("message").innerText = "Error: WalletConnect not loaded.";
        return;
      }
    ...
    document.getElementById("account").innerText = "Your address: " + userAccount;
    ...
    document.getElementById("message").innerText = "Error: " + (err.message || "Connection failed");
  }
}
...
async function fetchUserInfo() {
  if (contract && userAccount && document.getElementById("user-info")) {
    try {
      ...
      document.getElementById("message").innerText = "Error fetching information: " + err.message;
    }
  }
}
...
async function register() {
  ...
  if (!ethers.utils.isAddress(uplineAddress)) {
    document.getElementById("message").innerText = "Error: Upline address is invalid";
    return;
  }
  ...
  try {
    ...
    if (network.chainId !== 137) {
      document.getElementById("message").innerText = "Please switch to Polygon Mainnet.";
      return;
    }
    ...
    if (balance.lt(totalRequired)) {
      document.getElementById("message").innerText = "Insufficient balance. At least 350 MATIC + gas fee required.";
      return;
    }
    ...
    await tx.wait();
    document.getElementById("message").innerText = "Success: Registration completed";
    await fetchUserInfo();
    await displayTree();
  } catch (err) {
    document.getElementById("message").innerText = "Error: " + (err.reason || err.message || "Transaction failed");
  }
}
...
async function displayTree() {
  ...
  try {
    ...
    treeDiv.innerHTML = treeHTML || "No tree to display.";
  } catch (err) {
...
function saveAd() {
  if (userAccount && document.getElementById("message")) {
    if (userAccount.toLowerCase() !== creatorAddress.toLowerCase()) {
      document.getElementById("message").innerText = "Only the DApp creator can add advertisements!";
      return;
    }
...
setTimeout(() => {
  if (typeof window.ethers === 'undefined') {
    document.getElementById("message").innerText = "Error: ethers.js not loaded. Please check your internet connection.";
  }
}, 1000);