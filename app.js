...
function updateTexts() {
  ...
  document.getElementById("balance-debt").previousSibling.textContent = t.balanceDebt + ": ";
  document.getElementById("six-hour-balance").previousSibling.textContent = t.sixHourBalance + ": ";
}
...
async function connectWallet() {
  if (typeof window.ethers === 'undefined') {
    document.getElementById("message").innerText = translations[currentLanguage].errorConnection + "ethers.js not loaded.";
    return;
  }
...
async function fetchUserInfo() {
  if (contract && userAccount) {
    try {
      ...
      const balanceDebt = user.leftCount.toNumber() - user.rightCount.toNumber();
      document.getElementById("balance-debt").innerText = balanceDebt >= 0 ? `${balanceDebt} (Left heavier)` : `${Math.abs(balanceDebt)} (Right heavier)`;
      document.getElementById("six-hour-balance").innerText = user.cycleBalanceCount.toString();
    } catch (err) {
...
async function register() {
  ...
  try {
    ...
    if (network.chainId !== 137) {
      document.getElementById("message").innerText = translations[currentLanguage].errorNetwork;
      return;
    }
    ...
    if (balance.lt(totalRequired)) {
      document.getElementById("message").innerText = translations[currentLanguage].errorBalance;
      return;
    }
    ...
    document.getElementById("message").innerText = translations[currentLanguage].errorRegister + (err.reason || err.message || "Transaction failed");
  }
}
    ...
    try {
      ...
      let treeHTML = `<div class="tree-node" style="animation-delay: ${depth * 0.3}s;">
        <div class="node-box">
          ...
          <span class="node-id">ID: ${user.id.toString()}</span>
        </div>
